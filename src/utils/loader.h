...
private:
    ...
    static uint64_t align_addr_on_page(uint64_t &addr)
    {
        ...
        DPRINTF("align: %08" PRIx64 "\n", addr);
        offset = addr & (ps - 1);
        addr &= ~(ps - 1);

        DPRINTF("aligned: %08" PRIx64 ", off:%08" PRIx64 "\n", addr, offset);
    ...
    static int load_elf(int fd, DebugInitiator *bus, uint64_t *entry)
    {
        ...
        DPRINTF("Loading elf with %d sections\n", hdr.e_phnum);

        for (i = 0; i < hdr.e_phnum; i++) {
            T_phdr *ph = &phdr[i];

            if (ph->p_type == PT_LOAD) {
                DPRINTF("Loading elf segment, start:%08" PRIx64 ", size:%08" PRIx64 "\n",
                ...
                if (written < ph->p_filesz) {
                    EPRINTF("Only %" PRIu64 " bytes were written over %" PRIu64 ". Trying to write outside ram?\n", 
...
open_fail:
    ...
    static int load_image(const std::string & img_fn, DebugInitiator *bus, uint64_t load_addr, uint64_t *img_size)
    {
        ...
        DPRINTF("Loading image %s at 0x%" PRIx64 "\n", img_fn.c_str(), load_addr);
